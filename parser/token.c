...
t_elem	which_token(char *input)
{
	...
	ex_tok = existing_token(); // take array of all possible tokens
	inexistant.str = NULL;
	inexistant.type = 0;
	inexistant.len = 0;
	while (ex_tok && ex_tok->str) //while we don't find token in array and str != NULL
	{
		if (!ft_strncmp(input, ex_tok->str, ex_tok->len))//if we don't find differnce in input and ex_tok
			return (*ex_tok);
		++ex_tok;
	}
	return (inexistant);
